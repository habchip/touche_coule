#include "TestSG_553.h"
#include "SG_553.h"

CPPUNIT_TEST_SUITE_REGISTRATION(TestSG553);


void TestSG553::test_get_type() {
  SG553* arme = new SG553();
  CPPUNIT_ASSERT(arme->get_type() == SG_553);
}


void TestSG553::test_get_nb_tir_par_tour() {
  SG553* arme = new SG553();
  CPPUNIT_ASSERT(arme->get_nb_tir_par_tour() == 3);
}


void TestSG553::test_passe_ton_tour() {
  SG553* arme = new SG553();
  CPPUNIT_ASSERT(arme->passe_ton_tour() == false);
}


void TestSG553::test_to_string() {
  SG553* arme = new SG553();
  CPPUNIT_ASSERT(arme->to_string() == "SG_553");
}


void TestSG553::test_cree_msg() {
  SG553* arme = new SG553();
...
void TestSG553::test_attaque_ma_grille() {
  SG553* arme = new SG553();
  ...
  try {arme->attaque_ma_grille("A1", *grille2);}
    ...
    CPPUNIT_ASSERT(msg_err == "On ne fait pas d'attaque sur la grille du joueur 2 avec un SG 553!");
  }
}


void TestSG553::test_attaque_sa_grille() {
  SG553* arme = new SG553();
  SG553* arme2 = new SG553();
  ...
  try {arme->attaque_sa_grille("A1", *grille);}
    ...
    std::string msg_err(e.what());
    CPPUNIT_ASSERT(msg_err == "On ne fait pas d'attaque sur la grille du joueur 1 avec un SG 553!");
  }
}



void TestSG553::test_affiche_attaque() {
  SG553* arme = new SG553();
